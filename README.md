# Auto-MSN

Automatically open pre-saved MSN news links to earn Microsoft Rewards Points.

## Features

- Automatically opens MSN news links in sequence
- Customizable delays between tabs (3-15 seconds)
- Set number of tabs to open (tabs to open ≤ links count)

## How to Use
1. Login to Bing with your account using any browser
2. Visit the GitHub Pages using the same browser: https://abdeh24.github.io/auto-msn/
3. Press Start to start the automation tool:
<div align="center">
  <img src="https://api.pikwy.com/web/69f1f25a2e46e264cf36f874.jpg" style="width: 100%;height: 100%; border-radius: 10px;">
</div>

### Do it on your own system locally
1. Clone the repository:
   ```bash
   git clone https://github.com/abdeh24/auto-msn.git
   ```
2. Open `index.html` in your browser
3. Add your MSN news links to `src/links.txt` if needed (one URL per line)
4. Adjust delay and tab count, then click START

## Technical Details

- **Built with**: HTML, CSS, JavaScript
- **No Dependencies**: Runs entirely in your browser
- **No Data Collection**: All processing happens locally
- **Open Source**: GPL-3.0 License

## Privacy & Safety

- No personal data collection
- Links processed locally
- No account information stored
- No cookies or trackers
- Safe to use - verified by myself

## Important Notes

- **Affiliate Disclaimer**: This tool is not affiliated with Microsoft, Bing, Edge, or Rewards
- **Terms of Service**: Using automation tools may violate Microsoft Rewards ToS - use at your own risk
- **Alternative**: Consider reading articles manually for genuine engagement
- **No Warranty**: Use this tool at your own discretion

## License

Licensed under GPL-3.0 - see [LICENSE](LICENSE) for details.

---
If it exists, then there is
